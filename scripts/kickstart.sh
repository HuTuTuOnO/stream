...
(curl -fsSL https://raw.githubusercontent.com/HuTuTuOnO/stream/master/scripts/smartdns.sh | bash) || OUT_ERROR "部署 SmartDNS 失败！"
...
OUT_ALERT "[信息] 下载程序中"
rm -fr release
wget -O release.zip https://github.com/HuTuTuOnO/stream/releases/latest/download/release.zip || OUT_ERROR "下载失败！"
...
sed -i "s/HuTuTuOnO/$SECRET/" /etc/stream.json
...
OUT_ALERT "[提示] 您的 API 地址 http://$CURRENT:8888/stream?secret=$SECRET"
cd ~ && rm -fr release
exit 0
